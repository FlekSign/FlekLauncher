...
//
//  The whole page is drawn from the `FSAppModel` the row already holds, so it
//  is on screen the instant the sheet opens. No source in this build has a
//  detail endpoint behind it, so the row is all there is. It carries what the
//  repo's
...
final class FlekAppDetailModel: ObservableObject {
    ...
    /// Every source in this build is one the user added, and none of them has a
    /// detail page behind it — the listing is all a page will ever have to show.
    /// It goes through the same parser, which leaves plain text alone but still
    /// copes with a repo that puts markup in the field.
...
struct FlekAppDetailSheet: View {
    ...
    private var developer: String? {
        guard let name = app.app_developer, !name.isEmpty else { return nil }
        return name
    }

    private var photos: [String] { app.app_screenshots ?? [] }
            ...
            ZStack(alignment: .top) {
                ...
                ScrollView {
                    ...
                    .animation(reduceMotion ? .easeInOut(duration: 0.2)
                               ...
                               value: model.descriptionBlocks.count)
