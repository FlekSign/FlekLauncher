...
//  existing catalog view model for fetching/paging and the saved-repositories
//  store; only the UI is new:
...
struct FlekInstallerView: View {
    ...
    /// An app whose page the user opened. A repo's `app_id` is only its index
    /// in that repo's catalog, so it identifies the app only alongside the
    /// install URL — hence both in `id`.
    struct DetailTarget: Identifiable {
        let app: FSAppModel
        var id: String { "\(app.app_id)|\(app.install_url)" }
    }
    ...
    /// relaunch, so after a restart the installer falls back to the first
    /// source while still remembering the choice within a session.
    ...
    var body: some View {
        ...
        .task {
            ...
            if let repoURL = preselectRepoURL, let match = repos.first(where: { $0.sourceURL == repoURL }) {
                ...
                // Fresh launch with nothing to restore: land on whatever the user
                // added first. With no sources at all, `repository` stays nil and
                // the empty state below is what shows.
                selectedRepoID = first.id
                viewModel.repository = Self.source(for: first)
            }
        ...
        .sheet(item: $detailTarget) { target in
            FlekAppDetailSheet(
                ...
                onInstall: { overrides in
                    enqueueInstall(target.app, overrides: overrides)
                }
            )
        }
    ...
    private var barsBottomInset: CGFloat {
        ...
        return 8 + 52                        // pill top padding + pill height
    }
    ...
    private var searchResults: some View {
        ScrollView {
                ...
                ForEach(Array(repoSearch.sections.enumerated()), id: \.element.id) { sectionIndex, repoSection in
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        if repoSection.apps.isEmpty && repoSection.isLoading {
                            // A section's slot, held while its search runs.
                            ...
                            ForEach(Array(repoSection.apps.enumerated()), id: \.element.id) { index, app in
                                FlekInstallerRow(
                                    ...
                                    installState: LCInstallQueue.shared.item(for: app.install_url)?.installState,
                                    isCompleted: LCInstallQueue.shared.completedURLs.contains(app.install_url),
                                    onInstall: { installSearchResult(app) },
                                    onCancel: {
                                        LCInstallQueue.shared.cancel(url: app.install_url)
                                    },
                                    onOpen: {
            ...
            .padding(.top, barsTopInset)
            .padding(.bottom, listBottomInset)
            // Late results fill in the placeholder slot held for them.
            // Animating on the batch
    ...
    private func openPendingDetail(afterPresentation: Bool) async {
        ...
        detailTarget = DetailTarget(app: request.app)
    }
    ...
    private func installSearchResult(_ app: FSAppModel) {
        enqueueInstall(app)
    }

    /// Queues the download and install. Nothing gates it: every source in this
    /// build is one the user added themselves.
    private func enqueueInstall(_ app: FSAppModel,
